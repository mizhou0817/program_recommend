...
def get_major_white_list(input_str,path_to_save_major_list,api_key):
    ...
    user_prompt = f"""
    根据以下输入：{input_str}
    先去除掉用户不感兴趣或不喜欢或是觉的比较难的专业方向，然后总结提取或推断出用户可能感兴趣或有用的专业方向。注意，只要不是用户表现出不喜欢或不感兴趣或不考虑的专业方向，输入中涉及的专业信息都可以默认是用户感兴趣的专业方向。然后，在下面的专业列表中,选出与用户兴趣相关的专业:{all_major_strs}.请尽量找出多的相关或相近的专业,同时不要遗漏用户提到过的潜在感兴趣专业。
    ...
    又比如输入为:"'金融', '化学', '法律'"，则应该返回与金融，化学，法律相关的专业，比如返回：['会计与金融','化学','化学工程','法律与法律研究']
