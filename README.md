...
1. 获取大类专业并建立Faiss索引
2. 获取项目并建立Faiss索引
3：提取用户属于的意向专业小类，意向专业小类备注，意向专业大类，以及用户输入的其他信息
4: 根据提取到的用户输入信息，简历一个项目的黑名单和白名单，黑名单就是在之后不应该出现在推荐里的，白名单是可以出现在推荐里的
5. 根据用户输入的意向专业小类进行项目推荐：调用函数 get_program_from_intended_major()
6. 根据用户输入的意向专业小类其他信息进行项目推荐：调用函数 get_program_from_intended_major_other()
