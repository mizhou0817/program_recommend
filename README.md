...
4. 根据提取到的用户输入信息，建立一个项目的黑名单和白名单，黑名单就是在之后不应该出现在推荐里的，白名单是可以出现在推荐里的。这里有四个函数调用：program黑名单：get_program_black_list(),program白名单：get_program_id_white_list(),major大类黑名单：get_major_black_list(),major大类白名单：get_major_white_list()  
